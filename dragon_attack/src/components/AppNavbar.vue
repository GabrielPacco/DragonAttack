...
export default {
  ...
  computed: {
    currentProject() {
      // Asegura que $route.name esté definido antes de usar includes
      return this.$route.name && this.$route.name.includes('Dragon') ? 'dragon' : 'math';
    },
  },
  methods: {
    goToDragonAttack() {
      this.$router.push('/dragon-attack');
    },
    goToMathAdventure() {
      this.$router.push('/math-adventure');
    },
  },
};
</script>
